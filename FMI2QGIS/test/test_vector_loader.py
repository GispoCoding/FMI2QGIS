...
def test_download_airquality(
    tmpdir_pth, wfs_url, wfs_version, air_quality_sq, extent_sm_1, monkeypatch
):
    air_quality_sq.parameters["starttime"].value = datetime.strptime(
        "2020-11-05T19:00:00Z", Parameter.TIME_FORMAT
    )
    air_quality_sq.parameters["endtime"].value = datetime.strptime(
    ...
    loader = VectorLoader(
        "", tmpdir_pth, wfs_url, wfs_version, air_quality_sq, add_to_map
    )

    test_file = Path(plugin_test_data_path("airquality_small.xml.gz"))
    expected_output = Path(plugin_test_data_path("airquality.sqlite"))

    def mock_download_to_file(
        uri, output_dir: Path, output_name: str, *args, **kwargs
    ) -> Path:
    ...
    assert loader.path_to_file == Path(
        tmpdir_pth, "fmi_observations_airquality_hourly_simple_uuid_utf8.sqlite"
    )
    assert loader.path_to_file.exists()
    assert loader.path_to_file.stat().st_size == expected_output.stat().st_size


def test_construct_uri_airquality(
    tmpdir_pth, wfs_url, wfs_version, air_quality_sq, extent_lg_1
):
    air_quality_sq.parameters["starttime"].value = datetime.strptime(
        "2020-11-05T00:00:00Z", Parameter.TIME_FORMAT
    )
    air_quality_sq.parameters["endtime"].value = datetime.strptime(
        "2020-11-06T00:00:00Z", Parameter.TIME_FORMAT
    )
    air_quality_sq.parameters["timestep"].value = 60
...
def test_construct_uri_airquality2(
    tmpdir_pth, wfs_url, wfs_version, air_quality_sq, extent_lg_1
):
    air_quality_sq.parameters["starttime"].value = datetime.strptime(
        "2020-11-05T00:00:00Z", Parameter.TIME_FORMAT
...
@pytest.mark.skip(
    "This test fails if run together with other tests, individually works fine..."
)
def test_loader_with_invalid_parameters(
    new_project, tmpdir_pth, wfs_url, wfs_version, air_quality_sq, extent_lg_1
):
    # Without starttime
    air_quality_sq.parameters["endtime"].value = datetime.strptime(
        "2020-11-06T11:00:00Z", Parameter.TIME_FORMAT
    ...
    loader = VectorLoader(
        "", tmpdir_pth, wfs_url, wfs_version, air_quality_sq, add_to_map, max_features=1
    )
    result = loader.run()
    ...
    assert (
        loader.exception.bar_msg["details"]
...
def test_adding_layer_temporal_settings(new_project, vector_loader, air_quality_sq):
    ...
    layer: QgsVectorLayer = QgsProject.instance().mapLayer(
        list(vector_loader.layer_ids)[0]
    )
    assert layer.isValid()
