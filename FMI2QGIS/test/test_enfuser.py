...
def test_uri(
    tmpdir_pth, fmi_download_url, feedback: LoggerProcessingFeedBack, extent_sm_1
):
    ...
    uri = enfuser_loader._construct_uri(products, extent_sm_1, start_time, end_time)
    assert uri == (
        "https://opendata.fmi.fi/download?producer=enfuser_helsinki_metropolitan&param=AQIndex"
...
def test_download(tmpdir_pth, fmi_download_url, feedback, extent_sm_1, monkeypatch):
    ...
    expected_output_file = enfuser_loader.download(
        products, extent_sm_1, start_time, end_time
    )
